...
class splunk_spl:
    ...
    def query_usecase1(self, search_query):
        ...
        try:
            ...
            sid_body = \
                http.request(self.base_url + '/servicesNS/search/jobs?output_mode=json', 'POST',
            ...
            count = 1
            while True:
                url = self.base_url + '/servicesNS/search/jobs/%s/results?output_mode=json' % sid
    ...
    def query_by_app(self, search_query,app_name):
        ...
        try:
            ...
            # print search_query
            sid_body = \
                http.request(self.base_url + '/'+app_name+'/search/jobs?output_mode=json', 'POST',
                             headers={'Authorization': 'Splunk %s' % authToken},
                             body=urllib.urlencode({'search': search_query}))[1]
            print(sid_body)
            sid = json.loads(sid_body)['sid']
            ...
            count = 1
            while True:
                url = self.base_url + '/'+app_name+'/search/jobs/%s/results?output_mode=json' % sid
                print(url)
                response, content = http.request(
                    uri=url,
                    method='GET',
    ...
    def search_user_list_case21(self, id):
        """
            查询id所拥有的项目权限
        ...
        role_app=[]
        label=[]
        query='|`searchProjectsByUserChatbot("%s")`' % id[:-14]
    ...
    def search_user_list_case22(self, id, app_name,decide_list):
        """
            查询case2项目进度详情
    ...
    def search_user_list_case32(self, id, date,decide_list,decide_name):
        """
        ...
        result_title = self.query(query_title)
        print("#JOB_RESULT result_title=%s" % result_title)
        query = '|search `getRemainingTasks4Chatbot("%s", "%s")`  '% ("a",date)
          ...
          with open('./D-ctrl_Log.log', 'r+') as f:
            ...
            if log_date:
              ...
              if day_value.days >=180:
                  f.truncate(0)
          f.close()
    def search_user_list_case41(self, id):
          """
          ...
          for b in range(len(list_Addressee)):
            ...
            result_verify_system=self.query_by_app(query_verify_system,"ChatBot")
            list_verify_system=[]
           ...
           # print("#JOB_RESULT Pname=%s" %  Pname)
            ...
            for AppName in list_Addressee_APP:
              ...
              result[list_Addressee[b]+"@a"]=html1
              result1=result1+""" '%s':'%s',"""%(list_Addressee[b]+"@aa_%s" % AppName,html1)
    ...
    def search_user_list_case42(self, id):
        """
            判断case4输入日期和项目名是否正确 并查询splunk
        ...
        for b in range(len(list_Addressee)):
         ...
         # print("#JOB_RESULT Pname=%s" %  Pname)
          ...
          for AppName in list_Addressee_APP:
            ...
            result[list_Addressee[b]+"@a"]=html1
            result1=result1+""" '%s':'%s',"""%(list_Addressee[b]+"@a_%s" % AppName,html1)
        #list_accendant=["paul.guangcao.zhang1@outlook.com"]
        #result["list_accendant"]=list_accendant
        result="""{%s}"""%result1[:-1]
        result=json.dumps(result)
    ...
    def search_user_list_case43(self, id):
        """
            判断case4输入日期和项目名是否正确 并查询splunk
        ...
        for b in range(len(list_Addressee)):
         ...
         # print("#JOB_RESULT Pname=%s" %  Pname)
          ...
          result[list_Addressee[b]+"@a1"]=html1
...
if __name__ == '__main__':
    """
      主程序
    """
    user_id, assigned_group, assigned_to,description,close_notes,actual_start,activity_due = sys.argv[1:]
    decide_name=""
    ...
    if assigned_group == '12':
        ...
        user_id = 'a'
        splunk_spl().search_user_list_case21(user_id)
        
    elif assigned_group == '22':
        user_id = 'a'
        splunk_spl().search_user_list_case22(user_id, assigned_to,decide_list)
        ...
        user_id = 'a'
        splunk_spl().search_user_list_case32(user_id, assigned_to,decide_list,decide_name)
        ...
        splunk_spl().search_user_list_case41(user_id)
    elif assigned_group == '42':
        splunk_spl().search_user_list_case42(user_id)
    elif assigned_group == '43':
        splunk_spl().search_user_list_case43(user_id)
